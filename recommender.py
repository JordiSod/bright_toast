...
# %%
def rec2(list_of_descriptors, countries, provinces, regions, variety,
         min_price, max_price, wineset=wine_reviews_mincount, number_of_suggestions=10):
    '''
    Returns a dictionary with wine recommendations.
    Output includes the a recommendation number , a wine title,
    and a list of descriptors.
...
# Filter wineset by country, etc.
    ...
    if provinces != ['all']: wineset = wineset.loc[wineset['province'].isin(provinces)]
    if regions != ['all']: wineset = wineset.loc[wineset['region_1'].isin(regions)]
    if variety != ['all']: wineset = wineset.loc[wineset['variety'].isin(variety)]
...
# %%
# def rec3(list_of_descriptors,countries, provinces, regions, varieties,
#          min_price, max_price, wineset=wine_reviews_mincount, number_of_suggestions=10):

def rec3(list_of_descriptors,countries, min_price, max_price, wineset=wine_reviews_mincount, number_of_suggestions=10):
...
#    if provinces != ['All']: wineset = wineset.loc[wineset['province'].isin(provinces)]
#    if regions != ['All']: wineset = wineset.loc[wineset['region_1'].isin(regions)]
#    if varieties != ['All']: wineset = wineset.loc[wineset['variety'].isin(varieties)]
